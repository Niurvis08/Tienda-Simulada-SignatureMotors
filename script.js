...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateCartUI() {
        ...
        const checkoutBtn = document.getElementById('checkoutBtn'); // Obtener el botón Pagar
        let total = 0;
        ...
        if (!cart.length) {
            ...
            if(checkoutBtn) checkoutBtn.disabled = true; // Deshabilitar si está vacío
            return;
        } else {
            if(checkoutBtn) checkoutBtn.disabled = false; // Habilitar si hay items
        }
    ...
    processPaymentBtn.addEventListener('click', async () => {
        ...
        // 1. **CREAR COPIA DEL CARRITO ACTUAL PARA LA FACTURA**
        // Esto asegura que la factura usa los datos correctos ANTES de vaciar el array 'cart'.
        const itemsToInvoice = JSON.parse(JSON.stringify(cart)); 

        try {
            // Simular éxito de pago y notificar
            ...
            // 3. Vaciar carrito (SOLO DESPUÉS DE GENERAR EL PDF)
            cart = [];
            saveCartToStorage();
            updateCartUI();
            ...
            console.error('Error en pago o generación de PDF:', err);
            alert('Ocurrió un error procesando el pago o generando el PDF.');
        }
    });

    // MODIFICADA para aceptar la lista de items a facturar como argumento
    function generateInvoicePDF(clientName, itemsToInvoice) {
        try {
            ...
            if (!itemsToInvoice || itemsToInvoice.length === 0) {
                console.error('Lista de ítems para factura vacía o nula.');
                alert('No se pudo generar la factura: la lista de ítems estaba vacía.');
                return;
            }

            doc.setFontSize(14);
            doc.text('SIGNATUREMOTORS - RECIBO OFICIAL', 14, 20);
            doc.setFontSize(11);
            ...
            // Guardar
            const filename = `Factura-SignatureMotors-${Date.now()}.pdf`;
            doc.save(filename);
            
        } catch (err) {
            console.error('Error generando PDF', err);
            alert('No se pudo generar la factura en PDF. Asegúrate de tener la librería jsPDF correctamente cargada en tu HTML.');
        }
    }
